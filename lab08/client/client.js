...
const displayNewChat = (chat_message) => {
    if (chat_message.username === username) {
        // display on left
        const newMessage = `
        <div class="right"> <strong>You: </strong> ${chat_message.text} </div>`;
        qs('#chat').insertAdjacentHTML("beforeend", newMessage);
    } else {
        // display on right
        const newMessage = `
        <div class="left"> <strong>${chat_message.username}: </strong> ${chat_message.text} </div>`;
        qs('#chat').insertAdjacentHTML("beforeend", newMessage);
    }
};
